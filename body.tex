...
identified over a decade ago, no prior work has systematically compared
...
We set out to add new lines to these graphs. Keeping the observation 
space fixed, we ask: \textbf{how do a neural network and an LLM-guided
...
correctness was verified by 38/38 unit tests.
...
\item \textbf{Seed population}: We initialize with hand-designed CCAs
  implemented as source-code policies. For single-point training: AIMD, Copa, an
...
\item \textbf{Constant}: minimal seed (increment=1, multiple=1.0,
  intersend=0). The LLM receives no congestion-control structure.
...
converged on the same core control structure:
...
The four seed families encode different CCA architectures: the
...
and pacing provides rate-based structure. That all four converge on the same
BDP-scaled pacing mechanism is consistent with the LLM's pre-trained
...
The constant seed provides \emph{zero} congestion-control
structure (increment=1, multiple=1.0, intersend=0), yet all three
constant-seed runs produced BDP-scaling. One hypothesis is that the
LLM's pre-trained familiarity with TCP biases it toward this template;
we cannot distinguish this from a fitness-landscape attractor
...
and prior exploratory failures reveal systematic
failure modes:
...
A practical advantage of the source-code representation is human-readable output.
The best evolved policy uses named variables and clear control
flow, whereas PPO uses opaque weights. AlphaCC's output
can be inspected and debugged---enabling the mechanism analysis in
\S\ref{sec:convergence} that would be harder to extract
from a neural network, which exposes no named variables or explicit control flow.
...
We placed two new representations on Remy's generalization graphs---a
neural network (PPO) and an LLM-guided code evolution system
(AlphaCC)---alongside Remy's decision trees. For the main comparison,
