...
We evaluate on 99 log-spaced link rates from 0.1--100~packets/ms
(1--1000~Mbps), with RTT${}={}$150~ms, 2~senders, mean on/off
duration 5000~ms, and infinite buffers.
AlphaCC scores are averaged over 10 random seeds per rate.
...
policies can maintain persistent state across calls---enabling
phase transitions, bandwidth estimation, and history-dependent
decisions. The statefulness ablation
...
seed families covering distinct CCA design approaches:
\begin{itemize}
...
control flow, which made the mechanism analysis in
\S\ref{sec:convergence} possible. Extracting comparable structure
from PPO's opaque weight matrices would require additional
