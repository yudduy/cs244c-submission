...
\caption{Normalized score vs.\ link rate (50 log-spaced points, 1--1000~Mbps).
All single-point methods trained on 10~Mbps, 150~ms RTT, 2 senders.
AlphaCC single-pt (solid purple) peaks near training but degrades at
high rates like Remy~1x. AlphaCC multi-pt (dashed purple, 6 rates)
shows broader coverage from multipoint training; faint lines show all
9 diverse-seed runs. PPO (red, $\pm$1$\sigma$ across 6 brains)
generalizes well across the full range. Remy 1000x from the
Learnability study~\cite{sivaraman2014experimental} still degrades
despite maximal training breadth. Shaded: Remy 1x--20x training range.}
\label{fig:generalization}
\end{figure*}

\begin{table*}[t]
\centering
\caption{Normalized score across 9 link rates. Bold marks the best
score. Remy/PPO/AlphaCC-1pt: all trained on a single condition
(10~Mbps, 150~ms, 2 senders). AlphaCC-6pt: best multipoint run
(pac\_r1, trained on 6 rates). PPO: mean$\pm$std across 6 brains;
BBR/Copa/AIMD: 5 seeds.}
...
\begin{table}[t]
\centering
\caption{Diverse-seed evolution: 12 runs across 4 seed
families, evaluated on 9 link rates. Remy~1$\times$: $-$1.35
(Python eval). $\Delta$ = run mean $-$ Remy mean (negative = worse).
IFS: in-flight-proportional scaling present; Phase: phase machine present.
All 12 runs contain rate-based pacing (\S\ref{sec:convergence}).}
...
\caption{Distribution of 9-rate mean scores by seed family. Pacing seeds
are most reliable (closest to Remy). AIMD seeds have the worst median
performance. Dashed: Remy~1x ($-$1.35).}
...
\caption{Diverse-seed AlphaCC vs Remy across 9 link rates (4 families $\times$ 3 reps).
Pacing runs (blue) are closest to Remy; Copa (red) has the highest variance.
Most runs beat Remy at high rates (9/12 at 94.9~Mbps) but lose near training.}
...
\end{verbatim}
\caption{Core mechanism from 3 policies (one per seed family). All compute
\texttt{bdp = min\_rtt / rec\_ewma}, scale by $\sqrt{\text{bdp}}$, and
pace from receive rate. Names differ; structure is identical.}
\label{fig:mechanism}
\end{figure}
...
\caption{AlphaCC evolution trajectory (12 runs, 15 generations each). Y-axis:
multipoint training fitness (6-rate mean score, not the 9-rate evaluation
mean). Gray: all candidates; colored lines: per-run running best by seed
family; black dashed: global running best. Clipped to $[-8, 0]$; degenerate
outliers omitted. Most improvement in first 3 generations.}
\label{fig:evolution}
\end{figure}
...
(9.5, 59.8~Mbps). AIMD is the most consistent across conditions;
AlphaCC has the largest variance (best: $-$0.38; worst: $-$4.94).}
\label{fig:rtt}
\end{figure}
