...
observing that PPO does not outperform Remy, and were curious whether an
approach that produces \emph{readable code}---rather than opaque
weights---would generalize differently. The answer is nuanced:
...
Across 12~independent evolution runs from 4~seed families, AlphaCC
...
PPO std is across 6 independently trained brains.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/cpp_generalization}
\caption{Three representations trained at 10~Mbps, evaluated in Remy's
C++ simulator. Remy~1x peaks near training; AlphaCC degrades most
gradually at high rates; PPO generalizes worst.}
\label{fig:cpp_gen}
\end{figure}
...
Remy~1x peaks near training (best score $-$0.18 at 15~Mbps) but
degrades sharply at both extremes, reproducing the overfitting pattern
from the original paper~\cite{winstein2013tcp}.
