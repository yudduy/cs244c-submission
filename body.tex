...
\iffalse
Armaan: We need to redo this once we have the updated results in.
The answer is nuanced:
were curious whether an
approach that produces \emph{readable code}---rather than opaque
weights---would generalize differently. 
\textbf{Representation matters}, but no method dominates everywhere.
Remy~1x peaks near training (best at 5/9 rates) while AlphaCC's
in-flight scaling gives it flatter degradation at high rates (best at
4/9). PPO generalizes worst, beaten by both at all tested rates.
Across 12~independent evolution runs from 4~seed families, 
\fi

%% Separate the discussion about alphacc interpretation to another paragraph
AlphaCC
repeatedly rediscovered the same interpretable
design---BDP-proportional scaling and rate-based pacing---regardless
of starting point (12/12 convergence), but no systematic run surpassed
Remy on the aggregate mean.
...
RTT${}={}$150~ms, 2 senders.

\paragraph{Two simulators.}
Remy and PPO are trained and evaluated in Remy's original C++ simulator;
our primary results (Figure~\ref{fig:cpp_gen}, Table~\ref{tab:generalization})
use this authoritative implementation. AlphaCC policies are evolved in a
Python reimplementation of the simulator (\S\ref{sec:calibration}),
then hand-translated to C++ for the main comparison. We additionally
provide Python-simulator evaluations (Figure~\ref{fig:generalization})
that include all methods on an equal footing plus hand-designed baselines.
...
\textit{Training Simulator}        & C++ (original)    & C++ (original)    & Python (reimpl.)  \\
...
representations evaluated in Remy's original C++ simulator across 50
...
Table~\ref{tab:generalization} reports scores at 9 representative rates.
...
\iffalse
Armaan: Commented this out because we need to replace these results with updated CCAs and we cannot have interpolation -- we need to use link rates that we actually evaluated on.
\begin{table}[t]
\centering
\caption{Normalized score (C++ simulator) at 9 link rates. Bold = best
among single-point methods. PPO: mean of 6 brains (9-point eval,
interpolated at non-matching rates).}
\label{tab:generalization}
\small
\setlength{\tabcolsep}{5pt}
\begin{tabular}{@{}rrrr@{}}
\toprule
Mbps & Remy~1x & PPO & AlphaCC \\
\midrule
1.0  & $-$3.73 & $-$3.48 & \textbf{$-$3.29} \\
2.4  & \textbf{$-$2.23} & $-$3.48 & $-$3.35 \\
6.0  & \textbf{$-$0.89} & $-$1.78 & $-$0.82 \\
9.5  & \textbf{$-$0.51} & $-$1.02 & $-$0.92 \\
15.0 & \textbf{$-$0.18} & $-$0.75 & $-$0.83 \\
24.0 & $-$0.75 & $-$1.24 & \textbf{$-$0.84} \\
38.0 & $-$1.06 & $-$1.85 & \textbf{$-$1.06} \\
60.0 & $-$1.72 & $-$2.45 & \textbf{$-$1.21} \\
95.0 & $-$2.30 & $-$3.05 & \textbf{$-$1.55} \\
\bottomrule
\end{tabular}
\end{table}
...
degrades sharply at both extremes, reflecting the poor generalization also observed
in the original paper~\cite{winstein2013tcp}.

\iffalse
Armaan: need to either remove this or update with updated results
Among single-training-point methods (Table~\ref{tab:generalization}),
Remy~1x is best at 5/9 rates (near training, 2--15~Mbps), while
AlphaCC is best at 4/9 (high rates, 24--95~Mbps plus 1~Mbps). PPO
is worst at all~9 rates---its generalization degrades sharply outside
the training range. AlphaCC's in-flight scaling enables flatter
degradation: its score stays above $-$1.6 at 95~Mbps while Remy~1x
drops to $-$2.3 and PPO to $-$3.1. At high rates ($>$37~Mbps),
the wider-trained Remy 10x and 20x trees still outperform all
single-point methods (Figure~\ref{fig:generalization}).
\fi 
...
PPO degrades most steeply, worse than both Remy and AlphaCC at all
tested rates. AlphaCC's in-flight scaling adapts window increments to
link capacity, giving it the flattest curve at high rates but losing
near training where Remy's precise tree fit dominates.

\textbf{No method dominates everywhere.} Remy~1x is best near training
(5/9 rates), AlphaCC at high rates (4/9), PPO at none.

\textbf{PPO underperforms both.} PPO is worse than Remy at all~9
rates in the C++ simulator despite sharing the same observations,
...
determine whether this reflects a fundamental limitation of the
...
\item \textbf{Representation shapes generalization.} Remy peaks
  near training (best at 5/9 rates) but degrades sharply at both
  extremes. PPO generalizes worst, beaten by both at all~9
  rates. AlphaCC is best at 4/9 rates (high rates), with the
  flattest degradation curve. No method dominates everywhere.
