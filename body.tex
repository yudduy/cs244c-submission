...
AlphaCC policies are functions with the signature:
\begin{verbatim}
...
statefulness ablation (\S\ref{sec:statefulness_ablation}) partially addresses this
confound. For the main comparison (Figure~\ref{fig:cpp_gen}), we
...
constant seed provides \emph{no} congestion-control structure (the LLM
must invent everything), AIMD provides loss-based structure, and pacing
provides rate-based structure. That all three converge on the same
...
is closely similar: estimate BDP from \texttt{min\_rtt / rec\_ewma}, scale
...
generated the BDP-scaling mechanism early in evolution. This is
consistent with the LLM importing a familiar TCP-like template,
after which evolution mainly tunes parameters and preserves useful
structure.
...
\end{verbatim}
\caption{Core mechanism from 3 seed families. All compute BDP from min\_rtt/rec\_ewma and pace from receive rate. Core structure is shared.}
\label{fig:mechanism}
\end{figure}
...
degrades score from $-$0.87 to $-$3.34. In this
...
These failure modes suggest that AlphaCC behaves as
...
\item \textbf{Mechanistic convergence.} Across 12~independent
  ...
  This is consistent with the LLM's TCP prior biasing search,
  though we cannot rule out a fitness-landscape attractor.
