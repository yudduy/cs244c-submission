...
\paragraph{Training and evaluation simulators.}
Remy and PPO are trained and evaluated in the C++ simulator.
AlphaCC policies are evolved in a Python reimplementation
(\S\ref{sec:calibration}), then hand-translated to C++ for all
reported results. This translation preserves the same state variables,
thresholds, and control logic; correctness was verified by
38/38 unit tests comparing Python and C++ outputs.
...
\toprule
 & Remy & PPO & AlphaCC \\
\midrule
\textit{Representation}  & Decision tree      & Neural net        & Python source     \\
\textit{Optimizer}        & Tree search       & Proximal policy   & LLM evolution     \\
\textit{Training Simulator}        & C++ (original)    & C++ (original)    & Python (evolve) / C++ (eval) \\
...
onsistently decreases performance at training link rate, but can improve performance on out-of-distribution link rates


\subsection{AlphaCC-Specific Analysis}
\label{sec:ppo_results}
 
Figure~\ref{fig:generalization} shows the same comparison in our Python
reimplementation (\S\ref{sec:calibration}), which additionally includes
Remy 10x/20x trees, hand-designed baselines, and AlphaCC multipoint
variants that cannot be evaluated in the C++ simulator.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/generalization}
\caption{Python simulator evaluation. (a)~Three representations trained at 10~Mbps. (b)~Effect of training breadth and hand-designed baselines.}
...
The three seed families encode different CCA architectures: the
...
\item \textbf{Over-engineering}: Another failed run built a PID-style
  ...
  coefficients for 30 generations to co-tune.
\item \textbf{Degenerate mutations}: One exploratory run produced mutations
  scoring $-$105 (generation 27). We did not observe recovery from
...
\textbf{PPO.} PPO underperforms Remy across most of the rate range
...
(AlphaCC)---alongside Remy's decision trees, all trained on a single
condition (15~Mbps, 150~ms RTT, 2~senders) and evaluated across
