...
every evolved policy converged on the same core mechanism---BDP-proportional
...
\begin{table}[t]
\centering
\caption{Diverse-seed results (12 runs, 4 families). $\Delta$ = run mean $-$ Remy mean. Won: rates where run beats Remy~1x out of 9. IFS: in-flight scaling (BDP-proportional window adjustment) present. All 12 use rate-based pacing.}
...
BDP-scaled pacing---including the constant seed, which starts with
zero congestion-control logic---is consistent with the LLM's pre-trained
knowledge of TCP biasing search toward this design. We cannot
distinguish whether this reflects a genuine fitness-landscape attractor
or the LLM's inability to explore beyond familiar TCP templates.
...
in-flight-scaled pacing regardless of seed family. This is consistent
with the LLM's pre-trained knowledge of TCP biasing search
toward familiar designs. We cannot distinguish
