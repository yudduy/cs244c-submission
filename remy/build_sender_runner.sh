#!/bin/bash
# Build sender-runner (whisker-only) without autotools or LibTorch.
# Strips neural/evolved code paths — only WHISKER and POISSON senders.
set -e
...
# Step 2: Generate whisker-only sender-runner source
echo "=== Generating whisker-only sender-runner ==="
cat > src/sender-runner-whisker.cc << 'SRCEOF'
/* Whisker-only sender-runner — no LibTorch dependency.
   Only supports WHISKER and POISSON sender types. */
#include <cstdio>
#include <vector>
#include <string>
#include <limits>
...
enum class SenderType { WHISKER, POISSON };
...
int main( int argc, char *argv[] )
{
  ...
  if ( input_filename.empty() ) {
    fprintf( stderr, "Usage: %s if=<whisker_file> [link=X] [rtt=X] [nsrc=X] [on=X] [off=X]\n", argv[0] );
    exit( 1 );
  }
...
# Step 3: Compile core Remy sources
cd src
...
# Step 4: Compile whisker-only sender-runner
echo "=== Compiling sender-runner-whisker.cc ==="
clang++ -std=c++17 -O2 -I../protobufs $PROTO_CFLAGS $BOOST_INC -c sender-runner-whisker.cc -o sender-runner-whisker.o

# Step 5: Link
echo "=== Linking sender-runner ==="
OBJ_FILES=(memory.o memoryrange.o rat.o whisker.o whiskertree.o receiver.o random.o
           ...
           evaluator.o sender-runner-whisker.o)

clang++ -o sender-runner "${OBJ_FILES[@]}" ../protobufs/libremyprotos.a $PROTO_LIBS -lm

echo "=== BUILD SUCCESS ==="
ls -la sender-runner
echo ""
echo "Usage: ./src/sender-runner if=path/to/cca.179 link=0.946 rtt=150 on=5000 off=5000 nsrc=2"
