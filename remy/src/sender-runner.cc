...
enum class SenderType { WHISKER, POISSON, ALPHACC };
...
int main( int argc, char *argv[] )
{
  ...
  for ( int i = 1; i < argc; i++ ) {
    ...
    if ( arg.substr( 0, 7 ) == "sender=" ) {
      ...
      } else if ( type_str == "alphacc" ) {
        sender_type = SenderType::ALPHACC;
        fprintf( stderr, "Running AlphaCC sender\n" );
      }
      ...
      prng_seed = atoi( arg.substr( 5 ).c_str() );
      fprintf( stderr, "Setting PRNG seed to %u\n", prng_seed );
    }
  }

  seed_global_PRNG( prng_seed );
  ...
  if ( sender_type == SenderType::ALPHACC ) {
    AlphaCCRat alphacc;
    Evaluator< AlphaCCRat > eval( configuration_range );
    auto outcome = eval.score( alphacc, false, 10 );
    parse_outcome< Evaluator< AlphaCCRat >::Outcome >( outcome );
  } else {
