...
def evolved_policy(memory):
    ...
    if not hasattr(evolved_policy, '_state'):
        evolved_policy._state = {'cycle': 0, 'max_bw_est': 0.0, 'probe_rtt_rounds': 0}
    st = evolved_policy._state
...
class EvolutionConfig:
    ...
    duration_ms: float = 30_000.0  # sim duration per config
    num_trials: int = 1
    seed: int = 42
    model: str = "gpt-5.3-codex"
    output_dir: str = "results/python_sim/output_remy_evolve"
...
def run_evolution(config: EvolutionConfig) -> Dict:
    ...
    # Evaluate seeds
    print("=== Evaluating seed policies ===")
...
def run_evolution_multipoint(config: EvolutionConfig) -> Dict:
    ...
    # Evaluate seeds
    print("=== Evaluating seed policies (multi-point) ===")
    for name, code in SEEDS.items():
        fn = compile_policy(code)
        if fn is None:
            print(f"  Seed {name}: FAILED to compile")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--duration', type=float, default=30_000.0,
                        help='Simulation duration per config (ms)')
    parser.add_argument('--model', type=str, default='gpt-5.3-codex')
    parser.add_argument('--output', type=str, default='results/python_sim/output_remy_evolve')
